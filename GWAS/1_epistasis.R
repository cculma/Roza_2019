...
sep21 <- QTL_03 %>% distinct(Marker, .keep_all = T)
...
setwd("~/Documents/git/big_files/")
Y <- read.csv("yield_DS.csv", header = T)
colnames(Y)
head(Y)
Y <- Y %>% dplyr::select(Roza2019_VCF_ID, predicted.value)
Y1 <- na.omit(Y) 
...
for(seed in Seeds){
  ...
  fit.rf <- cforest_gen(formula = fit.rf.formula,
                        ...
                        control = ctree_control(teststat="quad", testtype="Univ", mincriterion=0.95, minsplit = 30, minbucket = 30),
                        mtry = q, cores = 10,
                        ntree = ntree) 
...
##Summarizing over the SNP Interaction Matrix across the 10 forest using the GenerateInteractionList.
All_Interactions_Stats_SRF <- GenerateInteractionList(Interaction_List_SRF, Importance_Score)

plotSNPInteraction(data2, c("chr5.1_42131042", "chr6.1_49779667"))
...
str(marks.1)
data2 <- data %>% mutate_if(is.integer, as.factor)
