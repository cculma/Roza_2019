...
pheno <- read.csv("BLUE_Yi_sqrt_DArT_DS.csv")

head(pheno)
trait1 <- colnames(pheno)[2:(length(colnames(pheno))-5)]
trait1
params <- set.params(fixed=c("PC1","PC2","PC3","Stress","Year"),
                     fixed.type= c("numeric","numeric","numeric","factor","factor"), n.PC = 3)
models_1 <- c("general", "additive", "1-dom", "2-dom",  "diplo-additive", "diplo-general")


data_1 <- read.GWASpoly(ploidy=4, 
                        pheno.file="BLUE_Yi_sqrt_DArT_DS.csv", 
