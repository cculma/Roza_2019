...
Y5 <- Y2 %>% dplyr::filter(env %in% c("total_20","total_21","total_22")) # only total
Y4 <- Y2 %>% dplyr::filter(!env %in% c("total_20","total_21","total_22")) # only indv harvest
...
levels(Y4$env)
Y4$env <- fct_relevel(Y4$env, c("may_20", "jun_20", "jul_20", "aug_20", "sep_20",
                                "may_21", "jun_21", "jul_21", "aug_21", "sep_21",
                                          "jun_22", "jul_22", "aug_22", "sep_22",
                                "may_23", "jun_23"))

ggplot(data = Y4, aes(x = env, y = predicted.values)) + geom_boxplot() + theme_bw(base_size = 12, base_family = "Arial") + theme(axis.text.x = element_text(angle = 90, hjust = 0.95, vjust = 0.2)) 

save.image("~/Documents/git/Roza_2019/BLUE_SpATS/BLUE.RData")
load("BLUE.RData")
